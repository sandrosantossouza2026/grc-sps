      ...
      <div style={{ width: '100%', maxWidth: 400, background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 16, padding: '40px 36px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            ...
            <input type="text" value={user} onChange={e => setUser(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSubmit()} placeholder="Digite seu usuário" autoComplete="username"
          ...
          <div>
            ...
            <input type="password" value={pass} onChange={e => setPass(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSubmit()} placeholder="Digite sua senha" autoComplete="current-password"
              style={{ width: '100%', padding: '11px 14px', background: 'var(--bg3)', border: `1px solid ${error ? 'var(--red)' : 'var(--border2)'}`, borderRadius: 8, color: 'var(--text)', fontSize: 13, outline: 'none' }} />
          </div>
          {error && (
            <div style={{ fontSize: 12, color: 'var(--red)', textAlign: 'center', background: 'var(--red-dim)', padding: '8px 12px', borderRadius: 6 }}>
